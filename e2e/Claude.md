...
2. **役割宣言の義務** - 各ステップに移行する際、必ず回答の冒頭に `[現在の役割: 〇〇]` と明記し、自身の権限と制限を自己確認してから作業を開始すること。
3. **自動テストの徹底と証拠保全** - 評価は必ず MCP Playwright を用いた実際のブラウザ操作で行う。失敗時は必ずエラーログ・コンソール出力等の証拠を `eval_report.md` に記録すること。推測による評価は厳禁。
4. **実行前の疎通確認** - Playwrightを実行する前に、必ずローカルサーバー（指定ポート）が応答するか確認すること。
5. **無限ループの防止（物理カウント）** - `progress.md` の先頭で `Loop Count: X` を管理し、差し戻し発生ごとに加算する。`X >= 3` に達した場合は即座にプロセスを停止し、ユーザーの介入を待つこと。
6. **起動手順の明記** - アプリの起動手順は常に最新の状態を保ち、Evaluator が Playwright を実行する前のセットアップで迷わないようにする。
```